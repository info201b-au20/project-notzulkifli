...
# black_cases_percent <- trunc(max(black_cases)/total_cases *100)
...
viz <- function(input) {
  plan <- RaceData %>% gather("type", "n", 3:ncol(RaceData)) %>% filter(!is.na(n)) %>% filter(State == "WA")
  #View(plan)
  ...
  v <- ggplot(filter_specific) + geom_line(aes(x = Date, y = n, group = type, color = type))
  v <- ggplotly(v)
  return(v)
}
